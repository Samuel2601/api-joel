...
const obtenerActividadProyecto = async function (req, res) {
    if (req.user) {
        var id = req.params['id'];
        try {
            let actividad = await Model.Ficha_sectorial.findById({ _id: id }).populate('encargado').populate('estado').populate('actividad');
...
const obtenerIncidenteDenuncia = async function (req, res) {
    if (req.user) {
        var id = req.params['id'];
        try {
            let incidente = await Model.Incidentes_denuncia.findById({ _id: id }).populate('categoria').populate('subcategoria').populate('ciudadano').populate('estado');
