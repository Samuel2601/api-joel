...
const listarUsuarios = async function (req, res) {
    if (req.user) {
        try {
            ...
            const carpeta = 'incidentes'; // Cambiar por la carpeta deseada
            const imagenes = listarImagenesEnCarpeta(carpeta);
...
const listarIncidentesDenuncias = async function (req, res) {
    if (req.user) {
        try {
            ...
            if(all){
                var incidentesDenuncias = await Model.Incidentes_denuncia.find(filtroConsulta).sort({ createdAt: -1 }).populate('categoria').populate('subcategoria').populate('ciudadano').populate('estado').populate('encargado');
            }else{                
...
function listarImagenesEnCarpeta(carpeta) {
  const rutaCarpeta = path.join(__dirname, '../uploads', carpeta);
  ...
  try {
    ...
    console.error(`Error al listar las imágenes en la carpeta ${carpeta}: ${error.message}`);
    return [];
  }
}
